...
with the next version using the `SemVer`_ number format. As of v4.0.0, this works
...
.. code-block:: diff
    ...
    # or with a type annotation (v4.0.0 or greater):
...
2. The variable name defined by ``variable`` and the version must be separated by
   ...
   As of v4.0.0, an optional Python-style type annotation (e.g. ``: str``,
